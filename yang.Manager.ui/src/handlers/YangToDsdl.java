...
//KLD: This handler is created to generate a .dsdl file from the YANG file.
                  ...
                  throws ExecutionException
                {
                  ...
                  finally
                  {
                      ...
                      namefile = namefile.substring(0, indexext); // name of file minus the extension
                          ...
                          String namefileout = filepath+ namefile + ".dsdl";
                          ...
                          MessageBox mb = new MessageBox(dlg.getParent(), 456);
                            ...
                            else if(result == 64) //Yes Selected
                            {
                              ...
                              dialog.setFilterNames(new String[] { "dsdl Files", "All Files (*.*)" });
                              dialog.setFilterExtensions(new String[] { "*.dsdl", "*.*" });
                              dialog.setFilterPath(filepath);
                              dialog.setFileName(namefile + ".dsdl");
                              dialog.open();
                          ...
                          try
                          {
                        	  ...
                        	  if(windows)
                        	  {
                        		  Findspace = Runtime.getRuntime().exec(new String[] {
                        				  "python",line, "-f", "dsdl", "-o", namefileout, pathfilein },null,
                      				  	f_path);
                        		  out.println("python " + line + " -f " +  "dsdl " + "-o " +  namefileout + " " + pathfilein);
                        	  }
                        	  else if(linux)
                        	  {
                        		  Findspace = Runtime.getRuntime().exec(new String[] {
                                		  "pyang", "-f", "dsdl", "-o", namefileout, pathfilein },null,
                      				  	f_path);
                        		  out.println("pyang" + " -f " +  "dsdl " + "-o " +  namefileout + " " + pathfilein);
                        	  }
                        	  else
                        	  {
                        		  IWorkbenchWindow windowErr = HandlerUtil.getActiveWorkbenchWindowChecked(event);
