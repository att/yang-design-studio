...
//KLD: This handler is created to generate a .xslt file for conversion of XML to JSON from the YANG file.
                  ...
                  throws ExecutionException
                {
                  ...
                  finally
                  {
                      ...
                      namefile = namefile.substring(0, indexext); // name of file minus the extension
                          ...
                          String namefileout = filepath+ namefile + ".xslt";
                          ...
                          MessageBox mb = new MessageBox(dlg.getParent(), 456);
                            ...
                            else if(result == 64) //Yes Selected
                            {
                              ...
                              dialog.setFilterNames(new String[] { "xslt Files", "All Files (*.*)" });
                              dialog.setFilterExtensions(new String[] { "*.xslt", "*.*" });
                              dialog.setFilterPath(filepath);
                              dialog.setFileName(namefile + ".xslt");
                              dialog.open();
                          ...
                          try
                          {
                        	  ...
                        	  if(windows)
                        	  {
                        		  Findspace = Runtime.getRuntime().exec(new String[] {
                        				  "python",line, "-f", "jsonxsl", "-o", namefileout, pathfilein },null,
                      				  	f_path);
                        		  out.println("python " + line + " -f " +  "jsonxsl " + "-o " +  namefileout + " " + pathfilein);
                        	  }
                        	  else if(linux)
                        	  {
                        		  Findspace = Runtime.getRuntime().exec(new String[] {
                                		  "pyang", "-f", "jsonxsl", "-o", namefileout, pathfilein },null,
                      				  	f_path);
                        		  out.println("pyang" + " -f " +  "jsonxsl " + "-o " +  namefileout + " " + pathfilein);
                        	  }
                        	  else
                        	  {
                        		  IWorkbenchWindow windowErr = HandlerUtil.getActiveWorkbenchWindowChecked(event);
